# from behave import when, then
#
#
# '''Successful Search'''
# @when(u'The Instructor types {name} into the Search Bar')
# def step_impl(context, name: str):
#     raise NotImplementedError(u'STEP: When The Instructor types C# into the Search Bar')
#
# @then(u'A list of results is displayed beneath the Search Bar')
# def step_impl(context):
#     raise NotImplementedError(u'STEP: A list of results is displayed beneath the Search Bar')
#
# @when(u'The Instructor clicks on a result')
# def step_impl(context):
#     raise NotImplementedError(u'STEP: The Instructor clicks on a result')
#
# @then(u'They are taken to the page for that Batch')
# def step_impl(context):
#     raise NotImplementedError(u'STEP: The Instructor clicks on a result')
